    ...
    async createOrder (symbol, type, side, amount, price = undefined, params = {}) {
        ...
        if (stopPrice === undefined && trigger === undefined) {
            if (contract) {
                ...
                const marginType = this.safeString2 (params, 'marginType', 'account', defaultMarginType);
                params = this.omit (params, [ 'account', 'marginType' ]);
                if (marginType === 'cross' || marginType === 'cross_margin') {
                    request['account'] = 'cross_margin';
                } else if (marginType === 'isolated' || marginType === 'margin') {
                    request['account'] = 'margin';
                } else {
                    request['account'] = 'spot';
                }
                if (timeInForce !== undefined) {
            ...
            if (contract) {
                ...
                request = {
                    ...
                    'put': {
                        ...
                        'price': this.priceToPrecision (symbol, price),
                        'amount': this.amountToPrecision (symbol, amount),
                        'account': market['margin'] ? 'margin' : 'normal',
